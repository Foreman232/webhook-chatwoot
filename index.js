...
const lastMessageMap = new Map(); // phone -> { id, timestamp }
...
// ✅ Webhook entrante de WhatsApp (360dialog)
app.post('/webhook', async (req, res) => {
  try {
    ...
    for (const msg of messages) {
      if (!msg || !msg.type || !msg.id) continue;
      ...
      else if (msg.type === 'image') content = msg.image?.link;
      else if (msg.type === 'audio') content = msg.audio?.link;
      else if (msg.type === 'video') content = msg.video?.link;
      else if (msg.type === 'document') content = msg.document?.link;
      else if (msg.type === 'location') {
        content = `📍 Ubicación: https://maps.google.com/?q=${msg.location.latitude},${msg.location.longitude}`;
      }

      await sendToChatwoot(conversationId, msg.type === 'text' ? 'text' : msg.type, content);

      try {
        await axios.post(N8N_WEBHOOK_URL, { phone, name, type: msg.type, content });
      } catch (n8nErr) {
        console.error('❌ Error n8n:', n8nErr.message);
      }
    }
...
app.post('/send-chatwoot-message', async (req, res) => {
  try {
    ...
    if (!contact || !contact.id) return res.status(500).send('No se pudo crear contacto');
    ...
    console.error('❌ Error reflejando mensaje desde Streamlit:', err.message);
