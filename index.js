...
// ✅ Vincular contacto al inbox (y obtener contact_inbox_id)
async function linkContactToInbox(contactId, phone) {
  try {
    ...
    return resp.data.id; // contact_inbox_id
  } catch (err) {
    if (err.response?.data?.message?.includes('has already been taken')) {
      const list = await axios.get(`${BASE_URL}/${CHATWOOT_ACCOUNT_ID}/contacts/${contactId}/contact_inboxes`, {
        headers: { api_access_token: CHATWOOT_API_TOKEN }
      });
      return list.data.payload[0]?.id;
    } else {
...
// ✅ Obtener o crear conversación con contact_inbox_id
async function getOrCreateConversation(contactId, contactInboxId, sourceId) {
  try {
...
app.post('/webhook', async (req, res) => {
  try {
    ...
    const conversationId = await getOrCreateConversation(contact.id, contactInboxId, contact.identifier);
